...
type APIKey interface {
	...
	FindActiveByHash(ctx context.Context, conn DBConn, keyHashHex string, now time.Time) (model.APIKey, error)
	...
	TouchLastUsedAt(ctx context.Context, conn DBConn, id int64, at time.Time) error

	Create(ctx context.Context, conn DBConn, k model.APIKey) (model.APIKey, error)
	GetByNaturalID(ctx context.Context, conn DBConn, idNatural string) (model.APIKey, error)
	List(ctx context.Context, conn DBConn, filter APIKeyListFilter, limit, offset int) (model.APIKeys, int, error)
	Update(ctx context.Context, conn DBConn, k model.APIKey) (model.APIKey, error)
	Revoke(ctx context.Context, conn DBConn, idNatural string, at time.Time) (model.APIKey, error)
}
